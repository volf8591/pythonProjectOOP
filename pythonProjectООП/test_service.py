...
class TestService:
    ...
    def start_test(student):
        ...
        for i in range(1, 6):
            try:
                question, unswer = Utils.generate_random_test()
    ...
    @staticmethod
    def show_everage_info(students):
        ...
        for name, data in students.items():
            ...
            to_print += f'{counter}. {name}: {sum(int_marks) / len(int_marks != 0 else "Нету отметок"}\n'
            counter += 1
