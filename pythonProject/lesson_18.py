...
MENU = {
    ADMIN_ACCESS : {
        1: 'Средний бал всех учеников и общий средний бал по школе',
        2: 'Перечень вех отметок по ученику',
...
def start_test(login):
    ...
    for i in range(1, 6):
        try:
            question, unswer = generate_random_test()
            print(f'Реши пример: {question}')
            user_answer = int(input('Ваш ответ: '))
            if unswer == user_answer:
                right_answers += 1
        except Exception as e:
            print('Вы ввели ерунду и потеряли шанс получить балы')
...
def run_study_program():
    ...
    if access_role == ADMIN_ACCESS:
        show_menu(access_role, login)
    elif access_role == STUDENT_ACCESS:
        show_menu(access_role, login)
    elif access_role == INVALID_CREDENTIALS:
        print('Ты не смог залогинится в программе')
        return
